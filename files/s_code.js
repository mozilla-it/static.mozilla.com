...
function setTrackerGen(page_s) {
	return function() {
		...
		s.linkTrackVars = 'prop8,prop11,prop16,events';
		s.linkTrackEvents = 'event1';

		if(a.dlParams) {
			s.linkTrackVars += ',eVar3,prop15';
			s.eVar3 = a.dlParams.product + ':' + a.dlParams.os + ':' + a.dlParams.lang;
			s.prop15 = s.eVar3;
		}

		s.prop8 = 'Download Button';
		if(page_s) {
			s.prop8 += " | " + page_s;
		}
		s.prop16 = s.pageName;
		s.events = 'event1';
		s.tl(a,'o','Download Link');

	};
}
function installOmniDownloadTrackers (regexp, page_s, tracker) {
	...
	for (var i = 0; dl = downloadLinks[i]; i++) {
		...
		if(window.addEventListener)
			dl.addEventListener("click", tracker, false);
		else
			dl.attachEvent("onclick", tracker);
	}
}

function TrackLinks(page_s, tracker) {
	installOmniDownloadTrackers("download.*?mozilla.org|download.html", page_s, tracker);
}
...
/*s_account is specified in a <script> tag above the s_code include*/

var s=s_gi(s_account)
/************************** CONFIG SECTION **************************/
/* You may add or alter any code config here. */
s.cookieLifetime="315569260"
/* Conversion Config */
s.currencyCode="USD"
...
s.queryVarsList="f"     // query parameters to keep
...
var plugins_ran = false;
var setTracker;
function s_doPlugins(s) {
	if (plugins_ran) {
		return true;
	}
	var re_download = new RegExp('download.html');
	var re_update   = new RegExp('whatsnew');
	var re_firstrun = new RegExp('firstrun');
	var re_entry    = new RegExp('^en-US$');
	var re_entry2   = new RegExp('^en-US/firefox$');
	...
	/* External Campaigns */
	if(!s.campaign){
		var campaign_vals = new Array(s.getQueryParam('utm_id'),
		                                  s.getQueryParam('utm_source'),
		                                  s.getQueryParam('utm_campaign'),
		                                  s.getQueryParam('utm_content'),
		                                  s.getQueryParam('utm_medium'));
		s.campaign=campaign_vals.join('-');
		s.campaign=s.campaign.replace(/^-+$/g, "");
		s.campaign=s.getValOnce(s.campaign,"s_cmp",0);
		s.eVar6=s.getQueryParam('utm_source');
	}	
	if(!s.campaign) {
		s.campaign=s.getQueryParam('from').toLowerCase();
	}
	if(!s.prop14) {
		s.prop14 = s.getQueryParam('product').toLowerCase();
	}
	if(!s.prop6) {
		var search_term = s.getQueryParam('query').toLowerCase();
		s.prop6 = search_term;
		s.eVar4 = search_term; 
	}
	if(!s.eVar8 && !s.eVar9) {
		s.eVar8 = s.getQueryParam('t');
		s.eVar9 = s.getQueryParam('uid');
	}

	if(!s.prop11) {
		s.prop11 = BrowserDetect.browser; 
	}

	if(!s.prop17) {
		s.prop17 = flashDetector();
	}

	if (re_download.test(s.pageName)) {
		s.prop8  = "Download Page";
		s.events = "event2";
	} else if (re_firstrun.test(s.pageName)) {
		s.prop8  = "Firstrun Page";
		s.events = "event9";
	} else if (re_update.test(s.pageName)) {
		s.prop8  = "Update Page";
		s.events = "event8";
	} else if (re_entry.test(s.pageName) || re_entry2.test(s.pageName)) {
		s.prop8  = "Entry Page";
		s.events = "event7";
	}
	setTracker = setTrackerGen(s.prop8);
	TrackLinks(s.prop8, setTracker);	
